                    ...
                    for instance in inst_it:
                        ...
                        if instance.name.lower().endswith('.mps'):
                            ...
                            if any(x in instance.name.lower() for x in ('spp', 'mis')):
                                exe = "../src/BBRVF_IP_biobj.py"
                                # Julia
                                if 'vOpt' in scenario:
                                    exe = "../src/vOpt/SPP_vOpt.jl"
                            elif 'kp' in instance.name.lower():
                                exe = "../src/BBRVF_IP_multiobj.py"
                                # Julia
                                if 'vOpt' in scenario:
                                    exe = "../src/vOpt/KP_vOpt.jl"
                            elif 'ip_obj_2' in instance.name.lower():
                                exe = "../src/BBRVF_IP_biobj_random.py"
                            elif 'ip_obj_' in instance.name.lower():
                                exe = "../src/BBRVF_IP_multiobj_random.py"
                            elif 'obj_2' in instance.name.lower():
                                exe = "../src/BBRVF_MILP_biobj.py"
                            else:
                                exe = "../src/BBRVF_MILP_multiobj.py"
                            command = ['python', exe, "-i", instance.path] + paramcmd.split()
                            # Julia
                            if 'vOpt' in scenario:
                                dat_file = instance.path[:-4]+'.dat'
                                command = ['julia', exe, "--instance", dat_file] + paramcmd.split()
