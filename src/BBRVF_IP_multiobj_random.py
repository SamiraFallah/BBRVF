...
parser.add_argument("-n", "--samples", help = "number of samples for zeta", default=80)
parser.add_argument("-f", "--frequencyrvf", help = "frequency of calling the RVF subproblem in main loop", default=0)
...
rvfFrequency = int(flags.frequencyrvf)
...
def RVFSubproblem(timelimit=None):
    ...
    totalTimeRVFiter += time.time() - st_rvf
...
totalTimeBBiter += time.time() - st_bb
...
for it_count, sample_zeta in enumerate(tempListZeta):
    mainIter += 1
    print('------------------------')
    ...
    shouldCallRVF = ((it_count + 1) % rvfFrequency) == 0 if rvfFrequency > 0 else gap_zeta <= 0.1            
...
print('Total Running Time RVFiter (sec): ', totalTimeRVFiter)
print('Total Running Time BBiter (sec): ', totalTimeBBiter)
print('EF: ' , EF)
